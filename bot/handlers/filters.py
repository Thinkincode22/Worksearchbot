...
async def filter_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "filters":
        ...
        current_city = user_search_state[user_id].get("city")
        await query.edit_message_text(
            "🏙️ Оберіть місто:",
        ...
        if city == "all":
            user_search_state[user_id].pop("city", None)
        else:
            user_search_state[user_id]["city"] = city
        await query.edit_message_text(
        ...
        current_category = user_search_state[user_id].get("category")
        await query.edit_message_text(
            "📋 Оберіть категорію:",
        ...
        if category == "all":
            user_search_state[user_id].pop("category", None)
        else:
            user_search_state[user_id]["category"] = category
        await query.edit_message_text(
        ...
        current_type = user_search_state[user_id].get("employment_type")
        await query.edit_message_text(
        ...
        if emp_type == "all":
            user_search_state[user_id].pop("employment_type", None)
        else:
            user_search_state[user_id]["employment_type"] = emp_type
        await query.edit_message_text(
        ...
        # Тут можна додати стан очікування введення зарплати
